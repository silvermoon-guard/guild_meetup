...
2. Run the gallery build so `index.html` reflects the change:
   ```bash
   node scripts/build-gallery.mjs
   ```
   Never hand-edit the block between `<!-- GALLERY:START -->` and
   `<!-- GALLERY:END -->` in `index.html` — that block is generated by the script and
   will be overwritten. Captions are controlled by file names or `images/captions.json`.
...
|---|---|
| `index.html` | The entire website — all text and layout. Edit this to change wording. |
| `images/` | Drop photos here. They become the gallery automatically. |
| `images/captions.json` | *(optional)* Custom captions for specific photos. |
| `scripts/build-gallery.mjs` | The little helper that rebuilds the gallery from `images/`. Runs automatically when publishing; you rarely run it by hand. |
